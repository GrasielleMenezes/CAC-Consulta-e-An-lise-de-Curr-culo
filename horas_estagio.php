<?php
    ...
    $db = mysql_select_db('analise');
            ...
            $verifica = mysql_query("SELECT horas.id_matricula AS 'MATRICULA', horas.descricao_estagio AS 'DESCRICAO', horas.horas_estagio AS 'QUANTIDADE DE HORAS', al.nome_aluno AS 'NOME DO ALUNO' FROM horas_estagio horas, alunos al WHERE (al.nome_aluno = '$login' AND al.senha = '$senha') AND (horas.id_matricula = al.matricula_aluno)") or die("erro ao selecionar");
		...
		$verifica2 = mysql_query("SELECT SUM(horas.horas_estagio) AS 'QUANTIDADE TOTAL DE HORAS DE ESTAGIO' FROM horas_estagio horas, alunos al WHERE (al.nome_aluno = '$login' AND al.senha = '$senha') AND (horas.id_matricula = al.matricula_aluno)") or die("erro ao selecionar");
