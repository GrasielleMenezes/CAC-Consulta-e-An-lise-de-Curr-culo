<?php
    ...
    $db = mysql_select_db('analise');
            ...
            $verifica = mysql_query("SELECT horas.id_matricula AS 'MATRICULA', horas.descricao AS 'DESCRICAO', horas.quant_horas AS 'QUANTIDADE DE HORAS', al.nome_aluno AS 'NOME DO ALUNO' FROM horas_complementares horas, alunos al WHERE (al.matricula_aluno = '$matricula' AND horas.id_matricula = '$matricula')") or die("erro ao selecionar");
                if (mysql_num_rows($verifica)==0) 
		{
        	...
        	$verifica2 = mysql_query("SELECT SUM(horas.quant_horas) AS 'QUANTIDADE TOTAL DE HORAS COMPLEMENTARES' FROM horas_complementares horas, alunos al WHERE (al.matricula_aluno = '$matricula' AND horas.id_matricula = '$matricula')") or die("erro ao selecionar");
